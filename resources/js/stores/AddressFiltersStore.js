...
export const useAddressFiltersStore = defineStore(
    "addressFilters",
    function () {
        ...
        const {
            data: addressesCountryOptions,
            isFinished: countriesFetched,
            execute: fetchCountries,
        } = useAxios("/api/countries", {}, { immediate: false });
        const {
            data: addressesCharacterOptions,
        ...
        const countriesWatch = watchPausable(
            [selectedAddressCountries],
            function ([countries]) {
                countries =
                    countries.length > 0 ? countries.join(",") : "default";
                let startDate;
                let endDate;
                if (
                    selectedAddressDates.value &&
                    selectedAddressTypes.value !== "users"
                ) {
                ...
                const games =
                    selectedAddressGames.value.length > 0
                        ? selectedAddressGames.value.join(",")
                        : "default";
                const type = selectedAddressTypes.value;
                const continents =
                    selectedAddressContinents.value.length > 0
                        ? selectedAddressContinents.value.join(",")
                        : "default";
                const name =
                    selectedAddressName.value !== ""
                        ...
                        : "default";
                const characters =
                    selectedAddressCharacters.value.length > 0
                        ? selectedAddressCharacters.value.join(",")
                        : "default";
        ...
        const charactersWatch = watchPausable(
            [selectedAddressCharacters],
            function ([characters]) {
                characters =
                    characters.length > 0 ? characters.join(",") : "default";
                let startDate;
                let endDate;
                if (
                    selectedAddressDates.value &&
                ...
                const continents =
                    selectedAddressContinents.value.length > 0
                        ...
                        : "default";
                const countries =
                    selectedAddressCountries.value.length > 0
                        ? selectedAddressCountries.value.join(",")
                        : "default";
                const name =
                    selectedAddressName.value !== ""
                        ? selectedAddressName.value
        ...
        watch([selectedAddressContinents], function ([continents]) {
            continents =
                continents.length > 0 ? continents.join(",") : "default";
        ...
        watch(
            [selectedAddressTypes, selectedAddressDates],
            function ([type, dates]) {
                let startDate;
                let endDate;
                if (
        ...
        watch(
            ...
            function (availableCountries, oldValue) {
                if (oldValue && availableCountries.length !== 0) {
                    const availableCountryCodes = availableCountries.data.map(
                        (country) => country.code,
                    );
                    selectedAddressCountries.value =
                        selectedAddressCountries.value.filter((code) =>
            ...
            function (availableCharacters, oldValue) {
                if (oldValue && availableCharacters.length !== 0) {
                    ...
                    selectedAddressCharacters.value =
                        selectedAddressCharacters.value.filter((id) =>
                            availableCharacterIds.includes(id),
                        );
                }
            },
            { immediate: false },
