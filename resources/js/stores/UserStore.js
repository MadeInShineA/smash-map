...
export const useUserStore = defineStore("user", function () {
    ...
    function fetchNotificationsCount() {
        axios
            .get("/api/users/" + user.data.id + "/notifications/count")
            .then((response) => {
    ...
    function subscribeToNotifications() {
        Echo.private(`notifications.` + user.data.id).listen(
            "NotificationEvent",
            (e) => {
                const audio = new Audio(
                    "/storage/audios/notification-sound.mp3",
                );
                audio.volume = 0.2;
                audio.play();
                toast.value.add({
                    severity: "info",
                    icon: e.imageUrl,
                    summary: e.notificationType + " for : " + e.gameTitle,
                    detail: e.message,
                    life: 7000,
                });
            ...
            beamsClient
                ...
                .then(() =>
                    beamsClient.setUserId(
                        "App.Models.User." + user.data.id,
                        beamsTokenProvider,
                    ),
                )
                .then(() => {
                    console.log("Beams client has started");
                })
                .catch(console.error);
        }
    }
    ...
    function getSettings(darkMode) {
        return axios
            .get("/api/users/" + user.data.id + "/settings")
            .then((response) => {
    ...
    async function updateSettings(settings) {
        ...
        const response = await axios.put(
            "/api/users/" + user.data.id + "/settings",
            settings,
            header,
        );
        if (response.status === 200) {
            ...
            user.data.settings.hasDistanceNotifications =
                settings.notifications.includes("hasDistanceNotifications");
            user.data.settings.distanceNotificationsRadius =
                settings.distanceNotificationsRadius;
            user.data.settings.address = {
                lat: settings.address.latitude,
                lng: settings.address.longitude,
            };
    ...
    async function updateDistanceNotificationsRadius(
        ...
        const response = await axios.put(
            "/api/users/" +
                user.data.id +
                "/settings/update-distance-notifications-radius",
            { distanceNotificationsRadius: distanceNotificationsRadius },
            header,
        );
        if (response.status === 200) {
            user.data.settings.distanceNotificationsRadius =
    ...
    function getProfileInformationByUsername(username, darkMode) {
        return axios
            .get("/api/users/user-profile?username=" + username)
            .then((response) => {
                return response.data;
            })
            .catch((error) => {
    ...
    async function updateProfileInformation(profileInformation) {
        const header = {
            headers: {
                Accept: "application/json",
                "Content-Type": profileInformation.profilePicture
                    ? "multipart/form-data"
                    : "application/json",
            },
        };
    ...
    async function checkAuthentication(darkMode) {
        return axios
            .get("/api/users/" + user.data.id + "/check-authentication")
            .then((response) => {
                return true;
            })
            .catch((error) => {
