...
const distanceNotificationsRadius = ref(
    userStore.user.data.settings.distanceNotificationsRadius,
);
const isCircleVisible = ref(
...
watch(
    ...
    (value) => {
        ...
        distanceNotificationsRadius.value =
            value.settings.distanceNotificationsRadius;
...
<template>
    <AddressFilterSidebar
        :sideBarVisible="sideBarVisible"
        @switchSideBarVisible="switchSideBarVisible"
    ></AddressFilterSidebar>
    <template v-if="addressStore.addressesFetched">
        <GoogleMap
            ...
            language="en"
            :map-type-control-options="{ mapTypeIds: ['roadmap', 'satellite'] }"
            style="width: 100%; height: 100%"
            :center="center"
            :zoom="zoom"
            :min-zoom="4"
            @click="openedInfoWindowIndex"
            :clickableIcons="false"
            :fullscreen-control="false"
        >
            <Circle
                ref="circleRef"
                :options="circleParameters"
                @click="clickCircleEvent"
            ></Circle>
            <InfoWindow
                ref="circleInfoWindowRef"
                class="info-window"
                id="circle-info-window"
            >
                <h3 class="info-window-title">Distance Notification Radius</h3>
                <div id="distance-notifications-radius-input-container">
                    <FloatLabel>
                        <InputNumber
                            id="distance-notifications-radius"
                            class="input"
                            v-model.number="distanceNotificationsRadius"
                            @click="
                                validationErrors.distanceNotificationsRadius =
                                    []
                            "
                            :min="1"
                            :max="2000"
                        ...
                        <Slider
                            class="input"
                            v-model="distanceNotificationsRadius"
                            :min="1"
                            :max="2000"
                            @click="
                                validationErrors.distanceNotificationsRadius =
                            ...
                            "
                        />
                        <label for="distance-notifications-radius"
                ...
                <div class="validation-errors">
                    <TransitionGroup name="errors">
                        <template
                            v-for="distanceNotificationsRadius in validationErrors.distanceNotificationsRadius"
                            :key="distanceNotificationsRadius"
                ...
                <div>
                    <Button
                        ...
                        plain
                        text
                        @click="updateDistanceNotificationsRadius"
                    ></Button>
                </div>
            </InfoWindow>
            <CustomControl
            ...
            >
                ...
                <Button
                    class="map-button-top"
                    @click="addressFiltersStore.clearFilters"
                ...
                <Button
                    class="map-button-top"
                    @click="addressFiltersStore.fetchAddressesWithFilters"
                    icon="pi pi-refresh"
                    rounded
                    aria-label="Refresh"
                />
            ...
            <MarkerCluster>
                <Marker
                    v-for="(address, i) in addressStore.addresses.data"
                ...
                >
                    ...
                    >
                        <div>
                            <template
                                v-if="
                                    openedInfoWindowIndex !== undefined &&
                                    openedInfoWindowIndex === i
                                "
                            >
                                <template v-if="address.users.length > 0">
                                    ...
                                    <template v-for="user in address.users">
                                        ...
                                        <div
                                            v-if="user.isModder"
                                            class="user-is-modder-container"
                                        >
                                            <Tag
                                                value="Modder"
                                                rounded
                                    ...
                                    <template v-for="event in address.events">
                                        ...
                                        <div class="event-game-attendees">
                                            ...
                                            <Button
                                                ...
                                                @click="
                                                    eventsStore.handleEventSubscription(
                                                        event,
                                                        darkMode,
                                                    )
                                                "
                                                :loading="
                                                    eventsStore.subscriptionLoading
                                                "
                                                icon="pi pi-bell"
                                                :class="{
                                                    active: event.user_subscribed,
                                                }"
                                                rounded
                                        ...
                                        <div class="event-datetime">
                                            <Chip
                                                :label="
                                                    event.timezone_start_date_time +
                                                    ' / ' +
                                                    event.timezone_end_date_time +
                                                    ' ' +
                                                    event.timezone
                                                "
                                                icon="pi pi-clock"
            ...
            <CustomControl
                :position="
                    responsiveMenuDisplayed ? 'LEFT_BOTTOM' : 'BOTTOM_CENTER'
                "
            >
                <Button
                    class="map-button-bottom margin-bottom-15-important"
                    @click="legendsVisible = true"
                    icon="pi pi-filter"
                    rounded
                    label="Legends"
                />
                <Sidebar
                    v-model:visible="legendsVisible"
                    position="bottom"
                ...
                >
                    <div id="games-legend">
                        ...
                        <div class="game-legend">
                            <div
                                class="color-box"
                                style="background-color: #b19eef"
                            ></div>
                            <div class="game-name">Rivals 2</div>
                        </div>
                        <div class="game-legend">
                            <div
                                class="color-box"
...
.event-bell-button,
.event-bell-button:hover,
.event-bell-button.active,
.event-bell-button:focus,
.p-button.event-bell-button:enabled {
...
.dark .poi-info-window div,
.dark .poi-info-window a {
    background-color: #1e1e1e;
    color: white;
}
