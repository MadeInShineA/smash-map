...
const getTimezone = async function(){
    try{
        timezone.value = await axios.get('/api/timezone')
    } catch(error){
        timezone.value = 'UTC'
    }
        ...
        <div class="event-container">
            <Card v-for="event in events.data" :key="event.id" class="event-card">
                ...
                <template #content>
                    ...
                    <div class="event-datetime"><Chip :label="event.start_date_time + ' / ' + event.end_date_time + ' ' + timezone" icon="pi pi-clock"></Chip></div>
                </template>
            </Card>
        </div>
