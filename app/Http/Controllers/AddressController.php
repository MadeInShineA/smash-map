        ...
        try {
            ...
            if ($request->has("startDate") && $request->has("endDate")) {
                ...
                if ($startDate !== "default" && $endDate !== "default") {
                    ...
                    $addresses->whereHas("events", function ($query) use (
                        $startDateUTC,
                        $endDateUTC,
                        $request,
                    ) {
            ...
            if ($request->has("continents")) {
                ...
                switch ($continents) {
                    ...
                    default:
                        ...
                        $addresses->whereHas("country", function ($query) use (
                            $continents,
                        ) {
                            $query->whereHas("continent", function (
                                $query,
                            ) use ($continents) {
                ...
                switch ($type) {
                    ...
                    case "followedEvents":
                        $addresses->whereHas("events", function ($query) use (
                            ...
                            $query->whereHas("subscribed_users", function (
                                $query,
                            ) use ($request) {
                                $query->where(
                                    "user_id",
            ...
            if ($request->has("characters")) {
                ...
                switch ($characters) {
                    ...
                    default:
                        ...
                        $addresses->whereHas("users", function ($query) use (
                            $characters,
                        ) {
                            $query->whereHas("characters", function (
                                $query,
                            ) use ($characters) {
            ...
            return $this->sendError(
                $error,
                ["An error occurred while retrieving the addresses E 009"],
                500,
            );
        }
    }
